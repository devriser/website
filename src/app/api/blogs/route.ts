    ...
    `,
      ...
      if (blog_post_table_create_res.status_code === 200) {
        ...
        const insert_blog_post_data: any = await query({
          db_name: db_name,
          query: `
        INSERT INTO BlogPost (title, summary, content, selectedRadio1, topics, bannerImage, createdAt, updatedAt, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
          values: [
            title,
...
export async function GET(request: NextRequest) {
  try {
    ...
    let queryStr = `SELECT * FROM BlogPost`;

    if (blogID) {
      queryStr += ` WHERE blogID = ?`;
    }

    if (isDraftParam) {
      queryStr += blogID ? ` AND status = 'draft'` : ` WHERE status = 'draft'`;
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!blogID) {
      return NextResponse.json(
        { ...ApiResponseFailed, message: "blogID is required for deletion." },
        { status: 400 }
      );
    }
    ...
    const delete_blog_post_data: any = await query({
      db_name: db_name,
      query: `DELETE FROM BlogPost WHERE blogID = ?`,
      values: [blogID],
    });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!blogID) {
      return NextResponse.json(
        { ...ApiResponseFailed, message: "blogID is required for updating." },
        { status: 400 }
      );
    }
