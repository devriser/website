    ...
    `,
      ...
      if (blog_post_table_create_res.status_code === 200) {
        ...
        const insert_blog_post_data: any = await query({
          db_name: db_name,
          query: `
        INSERT INTO Portfolio (title, summary, content, selectedRadio1, topics, bannerImage, createdAt, updatedAt, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
          values: [
            title,
...
export async function GET(request: NextRequest) {
  try {
    ...
    let queryStr = `SELECT * FROM Portfolio`;

    if (portfolioID) {
      queryStr += ` WHERE portfolioID = ?`;
    }

    if (isDraftParam) {
      queryStr += portfolioID
        ? ` AND status = 'draft'`
        : ` WHERE status = 'draft'`;
    }
  ...
  try {
    ...
    if (!portfolioID) {
      return NextResponse.json(
        {
          ...ApiResponseFailed,
          message: "portfolioID is required for deletion.",
        },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    if (!portfolioID) {
      return NextResponse.json(
        {
          ...ApiResponseFailed,
          message: "portfolioID is required for updating.",
        },
        { status: 400 }
      );
    }
