...
const deleteBlog = async (caseID: number) => {
  const response = await fetch(
    `http://localhost:3000/api/case-study?caseID=${caseID}`,
    {
      method: "DELETE",
    }
  );
...
const CaseStudy = () => {
  ...
  const { data: blogs, isLoading, isError } = useQuery("caseStudy", fetchBlogs);
  ...
  const handleDelete = (caseID: any) => {
    if (caseID) {
      setSelectedBlogID(caseID);
  ...
  const handleView = (caseID: any) => {
    const blog = blogs.find((b: any) => b.caseID === caseID);
    ...
    <div className="flex flex-col justify-between p-4 gap-4 h-full">
      <div className="flex justify-between items-center">
        <div className="flex-1">
          <FormInput type="search" dashboard placeHolder="Search case study" />
        </div>
        <div className="flex justify-end flex-[3]">
          <Button
            ...
            style="solid"
            as="NextLink"
            href={"/web-dashboard/home/case-study/create-case-study"}
          >
            Create Case Study
          </Button>
        </div>
                ...
                <div className="flex-1 flex justify-end relative  items-center">
                  ...
                  {action === index && (
                    <div className="bg-dashboard-table w-fit h-fit rounded-large  absolute -left-10 p-2 z-50 flex flex-col gap-3 border border-table-border ">
                      <Link
                        href={`/web-dashboard/home/case-study/edit-case-study?caseID=${blog.caseID}`}
                      ...
                      <div
                        className="flex items-center gap-1 cursor-pointer"
                        onClick={() => {
                          handleView(blog.caseID), setAction(null);
                        }}
                      >
                        <div className="bg-[#0075FF1A] p-1 rounded-large">
                          <TableViewIcon />
