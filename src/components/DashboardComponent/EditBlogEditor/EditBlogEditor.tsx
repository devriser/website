...
export default function EditBlogEditor({ blogID }: { blogID: string }) {
  ...
  const {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/addTopic");
      if (!response.ok) {
  ...
  } = useQuery(["blogPost", blogID], async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/blogs?blogID=${blogID}`
      );
    ...
    <div className="flex gap-4 p-4 h-full relative">
      <div className="flex-[5] flex flex-col gap-4 p-4 bg-dashboard-bg rounded-large h-full">
        ...
        {showTitleRequiredMessage && (
          <div className="text-red-500 text-sm">Please give blog a title*</div>
        )}
        <textarea
          value={summary}
        ...
        <Editor
          ...
          initialValue="<p>Start Writing the blog.</p>"
          init={{
            content_css: "",
            resize: false,
            height: "100%",
        ...
        {showEditorRequiredMessage && (
          <div className="text-red-500 text-sm">Blog content is required.*</div>
        )}
      </div>
