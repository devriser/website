...
export default function EditPortfolioEditor({
  portfolioID,
}: {
  portfolioID: string;
}) {
  ...
  const {
    ...
    try {
      const response = await fetch(
        "http://localhost:3000/api/add-portfolio-topic"
      );
      if (!response.ok) {
  ...
  } = useQuery(["blogPost", portfolioID], async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/portfolio?portfolioID=${portfolioID}`
      );
  ...
  useEffect(() => {
    ...
    console.log("blogPostIsLoading:", blogPostIsLoading);
    if (!blogPostIsLoading && blogPostData) {
      const {
        title: initialTitle,
        summary: initialSummary,
        content: initialContent,
        selectedRadio1: initialSelectedRadio1,
      ...
      setSelectedRadio1(initialSelectedRadio1 || 1);
      setSelectedTopics(initialTopics || []);
      setFilePath(initialBannerImage || null);
    }
  }, [blogPostIsLoading, blogPostData]);
    ...
    <div className="flex gap-4 p-4 h-full relative">
      <div className="flex-[5] flex flex-col gap-4 p-4 bg-dashboard-bg rounded-large h-full">
        ...
        <Editor
          ...
          initialValue="<p>Start Writing the Portfolio.</p>"
          init={{
            content_css: "",
            resize: false,
            height: "100%",
      ...
      <div className="flex flex-col w-full gap-4 flex-1 ">
        ...
        <div className="bg-dashboard-bg h-full p-2 rounded-large flex flex-col gap-6 overflow-auto">
          ...
          <div className="gap-3 flex flex-col w-fit overflow-auto">
            <div className="flex gap-4  items-center justify-between">
              <h3 className="text-secondary-reverse font-medium text-text-subtitle whitespace-nowrap ps-1">
                Banner Image
              </h3>
            ...
            <label className="cursor-pointer max-w-[15em] overflow-auto">
              <div className="w-fit overflow-auto flex flex-col gap-3 bg-dashboard-light-bg p-2 rounded-large whitespace-nowrap">
                <input
                  type="file"
