...
export const makeDirectory = async ({ pathToUpload }: { pathToUpload: any }) => {
    await fs.mkdir(pathToUpload, { recursive: true }, async (error) => {
        if (error) {
            if (error.code === "EEXIST") {

                const res_json = ApiResponseSuccess({ status_code: 200, message: "Directory create successfully!", data: "" })
                return res_json
            } else {
                const res_json = ApiResponseFailed({ status_code: 500, message: error.message })
                return res_json
            }
        }
        const res_json = ApiResponseSuccess({ status_code: 200, message: "Directory create successfully!", data: "" })
...
export const handleCatchError = (error: Error) => {
    ...
    const res_json = ApiResponseFailed({ status_code: 500, message: errorMessage })
    return res_json
}

export const deleteFileInDirectory = async ({ filePath }: { filePath: string }) => {
...
export const isFileSizeValid = ({ file, size_in_mb }: { file: File, size_in_mb: number }) => {
...
export const groupDataByKeys = (data: { [key: string]: any; }, keys: string[]): Record<string, any> => {
...
export const createOrUpdateTable = async ({ query, db_name }: { query: any, db_name: any }) => {
    ...
    if (layoutData_table_create.data.length === 0) {
        ...
        if (tabDescribe.status_code === 200) {
            if (tabDescribe.data.length > 0) {
                ...
                const modifiedColumns = existingColumns.filter((existingColumn: any) => {
                    const columnName = existingColumn.Field;
                    return sqlColumns.hasOwnProperty(columnName) && existingColumn.Type.toLowerCase() !== sqlColumns[columnName].toLowerCase();
                });

                if (modifiedColumns.length > 0) {
                    // Columns found to modify, generate ALTER TABLE MODIFY COLUMN query
                    const modifyColumnsQueries = modifiedColumns.map((modifiedColumn: any) => {
                ...
                const newColumns = Object.keys(sqlColumns).filter((column: any) => !existingColumns.map((existingColumn: any) => existingColumn.Field).includes(column));
                ...
                if (newColumns.length > 0) {
                    ...
                    const alterTableQuery = `ALTER TABLE ${db_name}.${tableName} ADD COLUMN ${colFinalArr.join(', ADD COLUMN ')}`;
                    const alter_table_create: any = await create_db_table({
                        db_name: db_name,
                        query: alterTableQuery
                    });
                    delete alter_table_create.data
                    const res_json = { ...alter_table_create, message: `Columns ${newColumns.join(", ")} added successfully.` }
                    return res_json;
                } else {
                    // No new columns or modified columns, table is up to date
                    const res_json = ApiResponseSuccess({ status_code: 200, message: `Table already exists and is up to date.` })
                    return res_json;
                }
            } else {
                // Handle the case when the DESCRIBE query returns no data
                const res_json = { ...tabDescribe, message: `Error describing table: No data returned.` }
                return res_json;
            }
        } else {
            return tabDescribe;
...
export const checkDatabaseIsExist = async ({ database_name }: { database_name: string }) => {
    const show_db_res = await create_db({
        query: `SHOW DATABASES;`
    });
    if (show_db_res.status_code === 200) {
        const { data } = show_db_res
        const getDatabase = data.filter((io: any) => io.Database.toLowerCase() === database_name.toLowerCase())
